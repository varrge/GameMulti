...
- 迁入方式：直接从用户提供 zip 解压到 `GameMulti/apps/web/`，作为正式仓库内的主站源码目录

## 后续前端改造唯一落点

以下改造统一只在 `apps/web/` 内实现，不再分散到临时目录或其他仓库：
...
- 当前执行基线：`develop` 基线要求下的正式仓库落点为 `GameMulti/apps/web/`
...
- 但代码与提交记录必须仍然落在 `GameMulti/apps/web/`
