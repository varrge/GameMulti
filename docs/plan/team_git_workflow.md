...
从论坛、主站、插件服务进入真实部署阶段开始，所有上线相关任务必须遵守以下规则：

1. **边实现边完善部署脚本**
   - 只要改动会影响启动、部署、联调、运维或回滚，就必须同步更新 `infra/deploy/`、`infra/compose/` 或相关脚本。
   - 不允许只改业务代码，留下“之后手工部署”的隐性步骤。

2. **边实现边完善环境变量模板**
   - 新增或改变环境变量时，必须同步更新对应 `.env.example`。
   - 涉及生产部署的变量，需要写清楚用途、示例值和是否为 secret。
   - secret 只能放模板占位，不能提交真实值。

3. **边实现边完善部署文档**
   - 新增服务或联调流程时，必须同步更新 `docs/deployment/` 下的 runbook/checklist。
   - 文档至少说明：前置条件、必填变量、启动命令、验证命令、回滚方式、常见故障。

4. **真实上线前必须有验收路径**
   - 每个部署改动必须提供本地或测试环境验证方式。
   - 能写入 smoke 的检查必须写入 smoke；不能自动化的检查必须写进 checklist。

5. **Discourse/论坛接入的特殊要求**
   - 论坛部署脚本、环境模板、GameMulti 联调配置必须一起维护。
   - 必须覆盖 `FORUM_ORIGIN`、`FORUM_SSO_SECRET`、`DISCOURSE_PROVIDER_SECRET`、SMTP、HTTPS、备份和恢复说明。
   - 服务器部署前必须先跑通本地论坛栈：镜像可拉取、容器可启动、HTTP 可访问、失败日志可复现。
   - 真实论坛联调完成前，不得声称论坛上线完成。

6. **生产与开发配置分离**
   - 开发环境可以继续使用 compose dev 配置。
   - 生产环境必须单独维护部署模板和上线文档，不能直接把本地开发 `.env` 当生产配置。

这条规则的目标是：后续每推进一步上线能力，仓库里都同时留下可复现的脚本、模板和操作说明，避免部署知识只停留在口头或本机环境里。
